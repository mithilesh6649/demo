...
class AppointmentController extends Controller
{
    public function AppointmentList()
    {
        if (Auth::user()->can('appointments_management')) {
            $AppointmentList = Appointment::with('AppointmentMetaData.AppointmentStatus', 'User', 'Nutritionist')->get();
    ...
    public function updateAppointment(Request $request)
    {
        ...
        $time = $request->appoinment_start_time;
        ...
        if ($Appointment->update()) {
            $AppointmentMetaData = AppointmentMetaData::where('appointment_id', $request->id)->first();
            ...
            $AppointmentMetaData->start_time = $request->appoinment_start_time;
            $AppointmentMetaData->end_time = $request->appoinment_end_time;
    ...
    public function updateZoomMeeting($accessToken)
    {
        ...
        $AppointmentMetaData = AppointmentMetaData::where('appointment_id', $allAppoinmentUpdateDataID)->first();
        ...
        try {
            $response = $client->request('PATCH', '/v2/meetings/' . $AppointmentMetaData->meeting_id, [
                ...
                'json' => [
                    "topic" => "Gena Healthx Appointments Updated",
                    "type" => 2,
                    "pre_schedule" => false,
            ...
            if (204 == $response->getStatusCode()) {
                ...
                //
                $appoinments = Appointment::where('id',$allAppoinmentUpdateDataID)->first();
                Notification::StoreNotificaiton(8,$appoinments['user_id'],'users',Auth::user()->id,'admin',0);

                Notification::StoreNotificaiton(8,$appoinments['invitee_id'],'web_users',Auth::user()->id,'admin',0);
    ...
    public function AppoinmentDatStore($zoomData)
    {
        ...
        if ($allAppoinmentData) {
            ...
            $Appointment->user_id = $allAppoinmentData['user_id'];
            $Appointment->invitee_id = $allAppoinmentData['nutritionist_id'];
            $Appointment->status = '1';
            if ($Appointment->save()) {
                ...
                if ($AppointmentMetaData->save()) {
                 ...
                 $Appdetails['id'] = $appointmentId;
                      //dispatch(new SendEmailJob($Appdetails));

                      // Add Appoinment In Notificaiton Table.......
                        //StoreNotificaiton($notification_id ='',$notification_to='',$notification_to_gurad='',$notification_from='',$notification_from_guard='',$read='')
                 Notification::StoreNotificaiton(7,$allAppoinmentData['user_id'],'users',Auth::user()->id,'admin',0);

                 Notification::StoreNotificaiton(7,$allAppoinmentData['nutritionist_id'],'web_users',Auth::user()->id,'admin',0);
 ...
 public function AppoinmentSeheduledStore($zoomData)
 {
    ...
    if ($allAppoinmentScheduledData) {
        ...
        $AppointmentMetaData = AppointmentMetaData::where('appointment_id', $allAppoinmentScheduledData['id'])->first();
        $AppointmentMetaData->appointment_time = $combinedDT;
        $AppointmentMetaData->meeting_id = $zoomData['id'];
        $AppointmentMetaData->appointment_join_url = $zoomData['join_url'];
        $AppointmentMetaData->appointment_response = json_encode($zoomData);
...
// return  $times = $this->create_time_range($start_time,$end_time, '1 mins');
    ...
    public function ScheduledAppointmentSendMail(Request $request,$id=null)
    {

        $appointmentId = '8'; //$request->appoinment_id;
