...
@section('title', 'Super Admin | Testimonials')
                        ...
                        <div class="card-main pt-3">
                            <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                                ...
                                @can('add_review')
                                    <a class="btn btn-sm btn-success add-advance-options"
                                        href="{{ route('client.review.add') }}">Add Testimonial</a>
                                @endcan
                            ...
                            <div class="">
                                <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($data as $GeneticTest)
                                            <tr>
                                                <td class="display-none"></td>
                                                <td>{{ $GeneticTest->title }}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @can('view_review')
                                                            <a class="action-button" title="View"
                                                                href="{{ route('client.review.view', ['id' => $GeneticTest->id]) }}"><i
                                                                    class="text-success fa fa-eye"></i></a>
                                                        @endcan


                                                        @can('edit_review')
                                                            <a href="{{ route('client.review.edit', ['id' => $GeneticTest->id]) }}"
    ...
    <script>
        ...
        $(document).on("click", ".delete-button", function() {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ route('client.review.delete') }}",
                        type: 'post',
                        data: {
                            id: id
                        },
