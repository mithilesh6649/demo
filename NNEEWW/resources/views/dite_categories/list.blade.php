    ...
    <div class="col-md-12">
                ...
                <div class="order_details">
                    <div class="card-main pt-3">
                        <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                            ...
                            @can('add_diet_category')
                            <a class="btn btn-sm btn-success add-advance-options"
                            href="{{ route('dite.category.add') }}">Add Diet Category</a>
                            @endcan
                            ...
                            <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('view_diet_category') || Gate::check('edit_diet_category') || Gate::check('delete_diet_category'))
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($allDiteCategories as $data)
                                    <tr>
                                        ...
                                        <td class="text-warning"><span
                                            ...
                                            <td>
                                                ...
                                                @can('view_diet_category')
                                                <a class="action-button" title="View"
                                                href="{{ route('dite.category.view', ['id' => $data->id]) }}"><i
                                                class="text-success fa fa-eye"></i></a>
                                                @endcan

                                                @can('edit_diet_category')

                                                <a href="{{ route('dite.category.edit', ['id' => $data->id]) }}"
                                                    title="Edit"><i class="text-warning fa fa-edit"></i></a>
                                                    @endcan

                                                    
                                                    
                                                    @can('delete_diet_category')
        ...
        //$('.delete-button').click(function(e) {
            ...
            // console.log({id});
                swal({
                    title: "Are you sure?",
                    text: "Are you sure you want to  delete this Diet Category ?",
                    type: "warning",
                    showCancelButton: true,
                }, function(willDelete) {
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('dite.category.delete') }}",
                            type: 'post',
                            data: {
                                id: id
                            },
                            ...
                            success: function(response) {
                                ...
                                if (response.success == 1) {
                                    ...
                                    $('#flash-message').html(
                                        ' Diet Category  Deleted Successfully');
