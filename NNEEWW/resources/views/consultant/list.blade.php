                                ...
                                <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            @if (Gate::check('edit_consultations') || Gate::check('view_consultations') )
                                            <th>Actions</th>
                                           @endif
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($allConsultants as $allConsultant)
                                            <tr>
                                                <td class="display-none"></td>
                                                <td>{{ $allConsultant->type ?? '--' }} 
                                                    @if($allConsultant->free == 1)
                                                     <span class="badge badge-pill badge-primary">Free</span>
                                                    @endif

                                                    @if(!empty($allConsultant->discount))
                                                     <span class="badge badge-pill badge-danger">{{$allConsultant->discount}} % Off</span>
                                                    @endif
                                                ...
                                                <td>
                                                    ...
                                                    <a class="action-button" title="View"
                                                        href="{{ route('consultation.view', ['id' => $allConsultant->id]) }}"><i
                                                            class="text-success fa fa-eye"></i></a>
                                                     @endcan
                                                        
                                                   @can('edit_consultations')
                                                       
                                                   <a href="{{ route('consultation.edit', ['id' => $allConsultant->id]) }}"
