                        ...
                        <div class="order_details">
                            <div class="card-main pt-3">
                                <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                                    <h3 class="mb-0">Our Teams</h3>
                                       @can('add_our_team')
                                    <a class="btn btn-sm btn-success add-advance-options" href="{{ route('add_our_team') }}">Add Team</a>
                                     @endcan
                                    ...
                                    <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Status</th>
                                                 @if (Gate::check('view_our_team') || Gate::check('edit_our_team') || Gate::check('delete_our_team'))
                                                <th>Action</th>
                                                @endif
                                        ...
                                        <tbody>
                                            @foreach ($AllSpecializations as $AllSpecialization)
                                                <tr>
                                                    <td class="display-none"></td>
                                                    <td>{{ $AllSpecialization->name }}</td>
                                                    ...
                                                    <td>
                                                       @can('view_our_team')
                                                        <a class="action-button" title="View"
                                                            href="{{ route('view_our_team', ['id' => $AllSpecialization->id]) }}"><i
                                                                class="text-success fa fa-eye"></i></a>
                                                            @endcan
                                                        
                                                   @can('edit_our_team')
                                                        <a href="{{ route('edit_our_team', ['id' => $AllSpecialization->id]) }}"
