...
@section('title', 'Super Admin | Exercises')
                            ...
                            <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                                ...
                                @can('add_exercises')
                                    <a class="btn btn-sm btn-success add-advance-options"
                                        href="{{ route('add_exercise') }}">Add Exercise</a>
                                @endcan
                                ...
                                <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            @if (Gate::check('view_exercises') || Gate::check('edit_exercises') || Gate::check('delete_exercises'))
                                                <th>Actions</th>
                                            @endif
                                    ...
                                    <tbody>
                                        @foreach ($allExercises as $allExercise)
                                            <tr>
                                                ...
                                                <td>{{ $allExercise->title ?? '--' }}</td>
                                                 <td>{{ $allExercise->calories_burnt ?? '--' }}</td>
                                                <td>{{ $allExercise->duration_in_minutes ?? '--' }}</td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @can('view_exercises')
                                                            <a class="action-button" title="View"
                                                                href="{{ route('view_exercise', ['id' => $allExercise->id]) }}"><i
                                                                    class="text-success fa fa-eye"></i></a>
                                                        @endcan


                                                        @can('edit_exercises')
                                                            <a href="{{ route('edit_exercise', ['id' => $allExercise->id]) }}"
                                                        ...
                                                        @can('delete_exercises')
                                                            <a class="action-button delete-button" title="Delete"
                                                                href="javascript:void(0)" data-id="{{ $allExercise->id }}"><i
