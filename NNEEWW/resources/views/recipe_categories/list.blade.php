                    ...
                    <div class="order_details">
                        <div class="card-main pt-3">
                            <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                                <h3 class="mb-0">Recipe Categories</h3>
                                 
                                
                                @can('add_recipe_category')
                                <a class="btn btn-sm btn-success add-advance-options"
                                href="{{ route('recipe.category.add') }}">Add Recipe Category</a>
                                @endcan
                                ...
                                <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            @if (Gate::check('view_recipe_category') || Gate::check('edit_recipe_category') || Gate::check('delete_recipe_category'))
                                            <th>Actions</th>
                                           @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @foreach ($RecipeCategories as $RecipeCategory)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @can('view_recipe_category')
                                                    <a class="action-button" title="View"
                                                        href="{{ route('recipe.category.view', ['id' => $RecipeCategory->id]) }}"><i
                                                            class="text-success fa fa-eye"></i></a>
                                                     @endcan
                                                        
                                                   @can('edit_recipe_category')
                                                       
                                                   <a href="{{ route('recipe.category.edit', ['id' => $RecipeCategory->id]) }}"
                                                    title="Edit"><i class="text-warning fa fa-edit"></i></a>
                                                    @endcan

                                                    
                                                    
                                                    @can('delete_recipe_category')
    ...
    <script>
        ...
        $(document).on("click", ".delete-button", function() {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ route('recipe.category.delete') }}",
                        type: 'post',
                        data: {
                            id: id
                        },
