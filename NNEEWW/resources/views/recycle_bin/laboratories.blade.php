...
@section('title', 'Deleted Laboratory')
                ...
                <div class="order_details">
                    <div class="card-main pt-3">
                        <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                            <h3 class="mb-0">Deleted Laboratory</h3>
                        </div>
                        <div class="">
                            @if (session('status'))
                            ...
                            <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('restore_laboratory') || Gate::check('permanent_deleted_laboratory')  )
                                        <th>Actions</th>
                                        @endif
                                ...
                                <tbody>
                                    <?php for ($i = 0; $i < count((is_countable($deletedLaboratories) ? $deletedLaboratories : [])); $i++) {?>
                                        <tr>
                                            <td class="display-none"></td>
                                            <td>{{ $deletedLaboratories[$i]->first_name ?? '--' }} {{ $deletedLaboratories[$i]->last_name ?? '--' }}</td>
                                            
                                            
                                            @if (Gate::check('restore_laboratory') || Gate::check('permanent_deleted_laboratory')  )
                                            <td>
                                              @can('restore_laboratory')
                                              <a class="action-button restore-button" title="Restore" href="javascript:void(0)"
                                              data-id="{{ $deletedLaboratories[$i]->id }}"><i
                                              class="text-success fa fa-undo-alt"></i></a>
                                              @endcan
                                              
                                              @can('permanent_deleted_laboratory')
                                              <a class="action-button delete-button" title="Permanent Delete"
                                              href="javascript:void(0)" data-id="{{ $deletedLaboratories[$i]->id }}"><i
...
<script type="text/javascript">
    $(".datatable").on("click", ".restore-button", function() {
        ...
        swal({
            ...
            if (willRestore) {
                $.ajax({
                    url: "{{ route('restore_laboratories') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $('#flash-message').html(
                                'Laboratory has been Restored Successfully');
                   ...
                   // swal("Cancelled", "Laboratory not Restored", "error");
            }
        });
    });
    $(".datatable").on("click", ".delete-button", function() {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ route('permanent_delete_laboratories') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    //swal("Cancelled", "Laboratory is Safe", "error");
            }
        });
    });
</script>
