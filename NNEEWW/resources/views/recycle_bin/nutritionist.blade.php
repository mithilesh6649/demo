...
@section('title', 'Deleted Nutritionist')
                        ...
                        <div class="order_details">
                            <div class="card-main pt-3">
                                <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                                    <h3 class="mb-0">Deleted Nutritionist</h3>
                                </div>
                                <div class="">
                                    @if (session('status'))
                                    ...
                                    <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                        <thead>
                                            <tr>
                                                ...
                                                @if (Gate::check('restore_nutrionists') || Gate::check('permanent_deleted_nutrionists')  )
                                        <th>Actions</th>
                                        @endif
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $deletedUsers[$i]->name ?? '--' }}</td>
                                                <td>{{ $deletedUsers[$i]->email ?? '--' }}</td>

                                                <td>
                                                @if (!empty($deletedUsers[$i]->phone_number))
                                                    (+{{ $deletedUsers[$i]->country_code }}){{ $deletedUsers[$i]->phone_number }}
                                                @else
                                                    --
                                                @endif
                                            </td>
                                                ...
                                                <td>
                                                    ...
                                                    @can('permanent_deleted_nutrionists')
    ...
    <script type="text/javascript">
        $(".datatable").on("click", ".restore-button", function() {
            ...
            swal({
                ...
                if (willRestore) {
                    $.ajax({
                        url: "{{ route('restore_nutritionist') }}",
                        type: 'post',
                        data: {
                            id: id
                        },
                        ...
                        success: function(response) {
                            if (response.success == 1) {
                                ...
                                $('#flash-message').html(
                                    'Nutritionist has been Restored Successfully');
                   ...
                   // swal("Cancelled", "Nutritionist not Restored", "error");
                }
            });
        });
        $(".datatable").on("click", ".delete-button", function() {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ route('permanent_delete_nutritionist') }}",
                        type: 'post',
                        data: {
                            id: id
                        },
