...
@section('title', 'Deleted Health Complaints')
                ...
                <div class="order_details">
                    <div class="card-main pt-3">
                        <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                            <h3 class="mb-0">Deleted Health Complaints</h3>
                        </div>
                        <div class="">
                            @if (session('status'))
                            ...
                            <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('restore_health_complaints') || Gate::check('permanent_deleted_health_complaints')  )
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                    ...
                                    <?php for ($i = 0; $i < count((is_countable($deletedUsers) ? $deletedUsers : [])); $i++) {?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @can('permanent_deleted_health_complaints')
...
<script type="text/javascript">
    $(".datatable").on("click", ".restore-button", function() {
        ...
        swal({
            ...
            if (willRestore) {
                $.ajax({
                    url: "{{ route('restore_health_complaints') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $('#flash-message').html(
                                'Health complaint has been Restored Successfully');
    ...
    $(".datatable").on("click", ".delete-button", function() {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ route('permanent_delete_health_complaints') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $("#flash-message").addClass("alert-danger");
                            $('#flash-message').html(
                                'Health Complaint has been Deleted Successfully');
