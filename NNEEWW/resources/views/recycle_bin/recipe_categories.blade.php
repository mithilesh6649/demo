...
@section('title', 'Deleted Recipe Categories')
                ...
                <div class="order_details">
                    <div class="card-main pt-3">
                        <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                            <h3 class="mb-0">Deleted Recipe Categories</h3>
                        </div>
                        <div class="">
                            @if (session('status'))
                            ...
                            <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('restore_recipe_categories') || Gate::check('permanent_deleted_recipe_categories')  )
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                    ...
                                    <?php for ($i = 0; $i < count((is_countable($deletedUsers) ? $deletedUsers : [])); $i++) {?>
                                        <tr>
                                            ...
                                            <td>
                                               ...
                                               @can('permanent_deleted_recipe_categories')
...
<script type="text/javascript">
    $(".datatable").on("click", ".restore-button", function() {
        ...
        swal({
            ...
            if (willRestore) {
                $.ajax({
                    url: "{{ route('restore_recipe_category') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $('#flash-message').html(
                                ' Recipe Category has been Restored Successfully');
    ...
    $(".datatable").on("click", ".delete-button", function() {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ route('permanent_delete_recipe_category') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
