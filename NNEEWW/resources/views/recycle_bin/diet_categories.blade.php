...
@section('title', 'Deleted Diet Categories')
                ...
                <div class="order_details">
                    <div class="card-main pt-3">
                        <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                            <h3 class="mb-0">Deleted Diet Categories</h3>
                        </div>
                        <div class="">
                            @if (session('status'))
                            ...
                            <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('restore_diet_categories') || Gate::check('permanent_delete_diet_categories')  )
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php for ($i = 0; $i < count((is_countable($deletedUsers) ? $deletedUsers : [])); $i++) {?>
                                        <tr>
                                            ...
                                            <td>{{ $deletedUsers[$i]->title ?? '--' }}</td>
                                            
                                            @if (Gate::check('restore_diet_categories') || Gate::check('permanent_delete_diet_categories')  )
                                            <td>
                                                @can('restore_diet_categories')
                                                ...
                                                @can('permanent_delete_diet_categories')
...
<script type="text/javascript">
    $(".datatable").on("click", ".restore-button", function() {
        ...
        swal({
            ...
            if (willRestore) {
                $.ajax({
                    url: "{{ route('restore_diet_category') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $('#flash-message').html(
                                ' Diet Category has been Restored Successfully');
    ...
    $(".datatable").on("click", ".delete-button", function() {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ route('permanent_delete_diet_category') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $("#flash-message").addClass("alert-danger");
                            $('#flash-message').html(
                                ' Diet Category has been Deleted Successfully');
