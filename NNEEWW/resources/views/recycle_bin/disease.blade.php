                        ...
                        <div class="order_details">
                            <div class="card-main pt-3">
                                <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                                    <h3 class="mb-0">Deleted Disease</h3>
                                </div>
                                <div class="">
                                    @if (session('status'))
                                    ...
                                    <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                        ...
                                        <tbody>
                                            <?php for ($i = 0; $i < count((is_countable($deletedDisease) ? $deletedDisease : [])); $i++) {?>
                                            <tr>
                                                <td class="display-none"></td>
                                                <td>{{ $deletedDisease[$i]->name }}</td>
                                            

                                                <td>

                                                    <a class="action-button restore-button" title="Restore" href="javascript:void(0)"
                                                        data-id="{{ $deletedDisease[$i]->id }}"><i
                                                            class="text-success fa fa-undo-alt"></i></a>



                                                    <a class="action-button delete-button" title="Permanent Delete"
                                                        href="javascript:void(0)" data-id="{{ $deletedDisease[$i]->id }}"><i
    ...
    <script type="text/javascript">
        $(".datatable").on("click", ".restore-button", function() {
            ...
            swal({
                ...
                if (willRestore) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success == 1) {
                                ...
                                $('#flash-message').html(
                                    'Disease has been Restored Successfully');
                    ...
                    swal("Cancelled", "Disease not Restored", "error");
                }
            });
        });
        $(".datatable").on("click", ".delete-button", function() {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ route('permanent_delete_disease') }}",
                        type: 'post',
                        data: {
                            id: id
                        },
                    ...
                    swal("Cancelled", "Disease is Safe", "error");
                }
            });
        });
    </script>
