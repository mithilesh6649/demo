                            ...
                            <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('restore_social_links') || Gate::check('permanent_deleted_social_links')  )
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                    ...
                                    <?php for ($i = 0; $i < count((is_countable($deletedUsers) ? $deletedUsers : [])); $i++) {?>
                                        <tr>
                                            ...
                                            <td>
                                               ...
                                               @can('permanent_deleted_social_links')
...
<script type="text/javascript">
    $(".datatable").on("click", ".restore-button", function() {
        ...
        swal({
            ...
            if (willRestore) {
                $.ajax({
                    url: "{{ route('restore_social_links') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $('#flash-message').html(
                                'Social Links has been Restored Successfully');
    ...
    $(".datatable").on("click", ".delete-button", function() {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ route('permanent_delete_social_links') }}",
                    type: 'post',
                    data: {
                        id: id
                    },
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $("#flash-message").addClass("alert-danger");
                            $('#flash-message').html(
                                'Social Links has been Deleted Successfully');
