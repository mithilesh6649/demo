                            ...
                            <table style="width:100%" id="exampleTable" class="table table-bordered table-hover datatable">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('restore_job') || Gate::check('permanent_deleted_job')  )
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php for ($i = 0; $i < count((is_countable($deletedUsers) ? $deletedUsers : [])); $i++) {?>
                                        <tr>
                                            ...
                                            <td>{{ $deletedUsers[$i]->job_title }}</td>
                                            <td>{{ $deletedUsers[$i]->department }}</td>
                                             @if (Gate::check('restore_job') || Gate::check('permanent_deleted_job')  )
                                            <td>
                                                @can('restore_job')
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            ...
                            $('#flash-message').html(
                                'Job has been Restored Successfully');
