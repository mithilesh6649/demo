    ...
    <div class="col-md-12">
                ...
                <div class="order_details">
                    <div class="card-main pt-3">
                        <div class="order_heading alert d-flex align-items-center justify-content-between mb-4">
                            <h3 class="mb-0">RDA Values</h3>
                            <div>
                                @can('add_rda')
                                <a class="btn btn-sm btn-success add-advance-options"
                                href="{{ route('rda_value_add') }}">Add RDA  </a>
                                @endcan  
                            </div>
                            ...
                            <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        @if (Gate::check('view_rda') || Gate::check('edit_rda') || Gate::check('delete_rda'))
                                        <th>Actions</th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($allRdaValues as $data)
                                    <!-- {{$data}} -->
                                    <tr>
                                  ...
                                  <!--       @if ($data->status == 1)
                                        ...
                                        <td class="text-warning"><span
                                            ...
                                            <td>
                                                ...
                                                href="{{ route('rda_value_view', ['id' => $data->id]) }}"><i
                                                class="text-success fa fa-eye"></i></a>
                                                @endcan

                                                @can('edit_rda')

                                                <a href="{{ route('rda_value_edit', ['id' => $data->id]) }}"
        ...
        //$('.delete-button').click(function(e) {
            ...
            // console.log({id});
                swal({
                    title: "Are you sure?",
                    text: "Are you sure you want to  delete this RDA   ?",
                    type: "warning",
                    showCancelButton: true,
                }, function(willDelete) {
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('rda.delete') }}",
                            type: 'post',
                            data: {
                                id: id
                            },
