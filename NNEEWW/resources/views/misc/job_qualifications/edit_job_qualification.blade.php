...
@section('title', 'Edit Job Qualification')
    ...
    <div class="col-md-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            @endif
            <form id="addJobQualficationForm" method="post" action="{{ route('update_job_qualification') }}">
              @csrf
              <div class="card-body">
                @if ($errors->any())
                  ...
                  <div class="row">
                      ...
                      <div class="form-group">
                        ...
                        <select name="job_industry_id" class="form-control" id="job_industry_id">
                          @foreach($jobIndustries as $jobIndustry)
                            <option {{ $jobIndustry->id == $jobQualification->job_industry_id ? 'selected' : '' }} value="{{ $jobIndustry->id }}">{{ $jobIndustry->name }}</option>
                          @endforeach
                        </select>
                    ...
                    <div class="col-sm-6">
                      <div class="form-group">
                        ...
                        <input type="hidden" name="id" value="{{ $jobQualification->id }}">
                        <input type="text" name="name" class="form-control" id="name" maxlength="100" value="{{ $jobQualification->name }}">
                        <div id ="industry_error" class="error"></div>
                        @if($errors->has('name'))
                    ...
                    <div class="col-sm-6">
                      <div class="form-group">
                        ...
                        <select name="status" class="form-control" id="status">
                          <option {{ $jobQualification->status ? 'selected' : '' }} value="1">{{ __('adminlte::adminlte.active') }}</option>
                          <option {{ !$jobQualification->status ? 'selected' : '' }} value="0">{{ __('adminlte::adminlte.inactive') }}</option>
                        </select>
