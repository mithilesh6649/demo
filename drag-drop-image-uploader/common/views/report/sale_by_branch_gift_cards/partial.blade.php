 ...
 @foreach ($salesbranch as $key => $report)
     ...
     <tr>
         ...
         @foreach ($branch as $bkey => $branch_name)
             <td class="table_td">
                 @if (isset($salesbranch[$key][$bkey]['demo']))
                     <?php

                     $total_sum = $total_sum + $salesbranch[$key][$bkey]['demo'];

                     ?>
                     {{ $salesbranch[$key][$bkey]['demo'] == 0 ? '-' : number_format((float) $salesbranch[$key][$bkey]['demo'], 3, '.', '') }}
                 @else
                     -
                 @endif
             </td>
