...
@section('title', 'Super Admin | Credit Card Report By Month')
@section('content_header')
@section('content')

 <?php

  $credit_card_report_by_month_month = Session::get('credit_card_report_by_month_month');
  $credit_card_report_by_month_year = Session::get('credit_card_report_by_month_year');
                ...
                <div class="card">
                    <div class="card-main">
                        <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
                            <h3>Credit Card Report By Month</h3>

                             <input type="hidden" id="credit_card_report_by_month_filter" start_month="{{$credit_card_report_by_month_month ==''?'': $credit_card_report_by_month_month }}" start_year="{{$credit_card_report_by_month_year ==''?'': $credit_card_report_by_month_year }}">
<!--
                            <div class="d-flex align-items-center justify-content-start">
                                <div class="card-header p-0" style="display: none; border: none;">
                                    ...
                                    <a href="#" data-toggle="collapse" data-target="#advanceOptions"
                                        class="advance-option-margin show-advance-options">Advance Options <i
                                            class="fa fa-caret-down"></i></a>
                                </div>
                                <span class="font-weight-bold"></span>

                                @can('download_credit_card_report_by_month_report')
                                    <a class="action-button download-report" title="Download Report">
                                        ...
                                        <div class="selected_date w-100">
                                            ...
                                            <div class="button_input_wrap">
                                                <div class="date_range_wrapper wrap-align-input"
                                                    ...
                                                    <select class="form-control month mr-2" name="month" id="month">
                                                        @foreach (['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'] as $monthNumber => $month_)
                                                            <option value="{{ ++$monthNumber }}"
                                                                @if ($monthNumber == date('m')) selected @endif>
                                                                {{ $month_ }}</option>
                                                        @endforeach
                                                    </select>
                                ...
                                <table style="width:100%" id="order-online-users-list"
                                    ...
                                    <thead>
                                        <!-- <tr>
                                            ...
                                            <th colspan="4">CC AMEX</th>
                                            <th colspan="4">CC VISA</th>
                                            <th colspan="4">CC MSTER</th>
                                            <th colspan="4">CC DINERS </th>
                                            <th colspan="4">PAYMENT GETWAY</th>
                                            <th colspan="4">DR KNET</th>
                                            <th colspan="4">MONTH TOTAL</th>
                                        </tr> -->
                                        <tr>
                                            <th class="order-first align-top sorting_disabled">S.NO</th>
                                            <th class="first align-top sorting_disabled">Branch</th>
                                    ...
                                    <tbody class="filter_date_show" id="orders_list">
                                        @include('report.daily-reports.credit-card-report.month_partial')
                                    </tbody>
                                </table>
                            </div>
                        </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            function filter(reset=null) {
                $.ajax({
                    url: "{{ route('credit-card-report-by-month.filter') }}",
                    method: 'post',
                    data: {
                        month: $('#month').val(),
                        year: $('#year').val(),
            ...
            $(document).on('click', '.download-report', function() {
                ...
                var url = "{{ url('reports/daily-reports/credit-card-report-by-month/download') }}";
    ...
    <script type="text/javascript">
        $(document).ready(function(){
             //Check sessions.........
      var check_session_value_parent = $('#credit_card_report_by_month_filter');
