 ...
 @foreach ($salesbranch as $key => $report)
     ...
     <tr>
         ...
         @foreach ($branch as $bkey => $branch_name)
             <td class="table_td">
                 @if (isset($salesbranch[$key][$bkey]['gross_sale']))
                     <?php

                     $total_sum = $total_sum + $salesbranch[$key][$bkey]['gross_sale'];

                     ?>
                     {{ $salesbranch[$key][$bkey]['gross_sale'] == 0 ? '-' : number_format((float) $salesbranch[$key][$bkey]['gross_sale'], 3, '.', '') }}
                 @else
                     -
                 @endif
             </td>
