...
@section('content')
<?php
   $sales_by_branch_discount_year = Session::get('sales_by_branch_discount_yearsss');
                           ...
                           <div class="selected_date w-100">
                              ...
                              <div class="button_input_wrap">
                                 ...
                                 <div class="apply_reset_btn">
                                    ...
                                    @can('download_sales_by_branch_gross_sale_monthly_report')  
                                    <button
                                       class="action-button download-cashdeposit btn btn-danger downloaded"
                                       title="Download Report"
                                       data-url="{{ route('sales-by-branch-gross-sale-monthly.download') }}"><i
                  ...
                  <div class="table-responsive" id="full">
                     <table style="width:100%" id="order-online-users-list"
                        ...
                        <tbody class="filter_date_show" id="orders_list">
                           @include('report.daily-reports.sales-report.sale_by_branch_gross_monthly.partial')
                        </tbody>
                     </table>
                  </div>
               </div>
...
<script>
   ...
   function filter_apply(reset = null) {
       ...
       $.ajax({
           url: "{{ route('sales-by-branch-gross-sale-monthly.filter') }}",
           method: "POST",
           data: {
               year: year,
               reset: reset
   ...
   $.ajax({
   url: "{{ route('total_gross_sale_monthly') }}",
   method: "POST",
   data: {
   year: year,
   },
