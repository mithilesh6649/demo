...
@section('title', 'Super Admin | Deleted Sales Reporting')
      ...
      <div class="card">
        <div class="card-main">
          <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
            <h3>Deleted Daily Sales Report (DSR)</h3>
          </div>
           ...
           @endif 
              ...
              @if(Gate::check('restore_dsr') || Gate::check('permanent_deleted_dsr')) 
                        ...
                        <td>
                            ...
                            @can('restore_dsr') 
                            <a class="action-button restore-button" title="Restore" href="javascript:void(0)"  branch-id="{{$data->branch_id}}"  data-date="{{$data->report_date}}"><i class="text-success fa fa-undo"  ></i></a>
                            @endcan

                             @can('permanent_deleted_dsr')
                            <a class="action-button delete-button " title=" Permanent Delete" href="javascript:void(0)" branch-id="{{$data->branch_id}}"  data-date="{{$data->report_date}}"><i class="text-danger fa fa-trash-alt" ></i></a>
                           @endcan
                        </td>
        ...
        $('#select_branch').on('change',function(){
          var date_range = $('input[name="date_range"]').val('');
            var branch_id = $(this).val();
              $.ajax({
              url:"{{route('daily-sales.deleted.filter')}}",
              method:"POST",
              data:{
                    'branch_id':branch_id,
                    'with':'only_branch_id'
        ...
        $('.reset-button').on('click',function(e){
            ...
            e.preventDefault();
             ...
             $('#select_branch').val(0);
                   ...
                   $.ajax({
              url:"{{route('daily-sales.deleted.reset')}}",
              method:"POST",
              data:{
                    'branch_id':branch_id,
                    },
...
//Restore Users.........    
   ...
   $(document).on('click','.restore-button',function(){
      ...
      swal({
        title: "Are you sure?",
        text: "Are you sure you want to restore this Daily Sales Report (DSR) ?",
        type: "warning",
        showCancelButton: true,
      }, function(willDelete) {
        if (willDelete) {
          $.ajax({
            url: "{{route('daily-sales.restore')}}",
            type: 'post',
            data: {
              dates:dates,
              branch_id:branch_id,  
            ...
            success: function(response) {
              ...
              if(response.success == 1) {
                   ...
                   $('#flash-message').html('Daily Sales Report(DSR)  Restore Successfully');
...
//Permanent Delete
   ...
   $(document).on('click','.delete-button',function(){
      ...
      swal({
        ...
        if (willDelete) {
          $.ajax({
            url: "{{route('daily-sales.delete.permanently')}}",
            type: 'post',
            data: {
              dates:dates,
              branch_id:branch_id,  
