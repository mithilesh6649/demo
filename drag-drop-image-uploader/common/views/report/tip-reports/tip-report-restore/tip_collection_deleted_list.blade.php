                ...
                <div class="card">
                    <div class="card-main">
                        <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
                            <h3>Deleted Tip Reports ( Tip Collections )</h3>
                            <div class="collections_tabs d-flex align-items-center">
                                ...
                                <div class="right pl-2 d-none">
                                    <a class="btn btn-sm btn_clr btn-success added_content"
                                        href="{{ route('deleted.tip.report.list') }}">Deleted Tip Distribution</a>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#select_branch').on('change', function() {
                ...
                $.ajax({
                    url: "{{ route('tip.collection.report.deleted.filter') }}",
                    method: "POST",
                    data: {
                        'branch_id': branch_id,
                        'with': 'only_branch_id'
            ...
            $('.reset-button').on('click', function(e) {
                ...
                $.ajax({
                    url: "{{ route('tip.collection.report.deleted.reset') }}",
                    method: "POST",
                    data: {
                        'branch_id': branch_id,
                    },
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.restore-button', function() {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Are you sure you want to restore this Tip Collection Report ?",
                    type: "warning",
                    showCancelButton: true,
                }, function(willDelete) {
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('tip.collection.report.restore') }}",
                            type: 'post',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                if (response.success == 1) {
                                    ...
                                    $('#flash-message').html(
                                        ' Tip Collection Report  Restore Successfully'
                                        );
            ...
            $(document).on('click', '.delete-button', function() {
                ...
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('tip.collection.report.delete.permanently') }}",
                            type: 'post',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                if (response.success == 1) {
                                    ...
                                    $("#flash-message").addClass("alert-danger");
                                    $('#flash-message').html(
                                        ' Tip Collection Report  Deleted Successfully'
                                        );
                                    obj.parent().parent().remove();
                                    setTimeout(() => {
                                        $("#flash-message").addClass("d-none");
