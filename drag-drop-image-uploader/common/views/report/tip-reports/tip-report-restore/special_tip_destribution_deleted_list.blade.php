                ...
                <div class="card">
                    <div class="card-main">
                        <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
                            <h3>Deleted Tip Reports ( Special Tip Distribution )</h3>
                            <div class="collections_tabs d-flex align-items-center">
                                <div class="left mb-3">
                                    <a class="btn btn-sm btn_clr btn-success"
                                        href="{{ route('deleted.tip.report.list') }}">Deleted Tip
                                        Collections</a>
                                </div>
                                <div class="right pl-2">
                                    <a class="btn btn-sm btn_clr btn-success added_content"
                                        href="{{ route('deleted.tip.report.rider.list') }}">Deleted Tip Rider</a>
                                </div>
                                <div class="right pl-2">
                                    <a class="btn btn-sm btn_clr btn-success added_content"
                                        href="{{ route('deleted.special.tip.distribution.report.list') }}">Deleted Special
                                        Tip Distribution</a>
                                </div>
                                <div class="right pl-2 d-none">
                                    <a class="btn btn-sm btn_clr btn-success added_content"
                                        href="{{ route('tip-distributions.lists') }}">Deleted Tip Distribution</a>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="left_inner">
                                        ...
                                        <div class="button_input_wrap">
                                            ...
                                            <div class="date_range_wrapper wrap-align-input">
                                                ...
                                                <input type="text" class="input-wrap w-100 col-10" name="date_range"
                                            ...
                                            <div class="apply_reset_btn">
                                                ...
                                                <button class="btn btn-primary reset-button mr-1"
                                            ...
                                            @if (Gate::check('restore_maintenance_report') || Gate::check('permanent_delete_maintenance_report'))
                                                <td>
                                                    ...
                                                    @can('restore_maintenance_report')
                                                        <a data-id="{{ $data->id }}" class="action-button restore-button"
                                                            title="Restore" href="javascript:void(0)"
                                                            branch-id="{{ $data->branch_id }}"
                                                            data-date="{{ $data->distribution_date }}"><i
                                                                class="text-success fa fa-undo"></i></a>
                                                    @endcan

                                                    @can('permanent_delete_maintenance_report')
                                                        <a data-id="{{ $data->id }}" class="action-button delete-button "
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#select_branch').on('change', function() {
                ...
                $.ajax({
                    url: "{{ route('special.tip.distribution.report.deleted.filter') }}",
                    method: "POST",
                    data: {
                        'branch_id': branch_id,
                        'with': 'only_branch_id'
            ...
            $('.reset-button').on('click', function(e) {
                ...
                $.ajax({
                    url: "{{ route('special.tip.distribution.report.deleted.reset') }}",
                    method: "POST",
                    data: {
                        'branch_id': branch_id,
                    },
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.restore-button', function() {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Are you sure you want to restore this  Special Tip Distribution  ?",
                    type: "warning",
                    showCancelButton: true,
                }, function(willDelete) {
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('special.tip.distribution.report.restore') }}",
                            type: 'post',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                if (response.success == 1) {
                                    ...
                                    $('#flash-message').html(
                                        ' Special Tip Distribution   Restore Successfully'
                                        );
            ...
            $(document).on('click', '.delete-button', function() {
                ...
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('special.tip.distribution.report.delete.permanently') }}",
                            type: 'post',
                            data: {
                            ...
                            success: function(response) {
                                ...
                                if (response.success == 1) {
                                    ...
                                    $("#flash-message").addClass("alert-danger");
                                    $('#flash-message').html(
                                        ' Special Tip Distribution Deleted Successfully'
                                        );
                                    obj.parent().parent().remove();
                                    setTimeout(() => {
                                        $("#flash-message").addClass("d-none");
