...
@section('title', 'Super Admin | Deleted Branches')
          ...
          <table style="width:100%" id="users-list" class="table table-bordered table-hover yajra-datatable">
            <thead>
              <tr>
               ...
               @if(Gate::check('restore_branch') || Gate::check('permanent_deleted_branch'))
               <th>Actions</th> 
               @endif
           ...
           <tbody>
            @foreach($branchesList as $key => $data)
            <tr> 
              <td> {{ $data->title_en  ?? 'N/A'}}</td>
              <td>{{ $data->title_ar  ?? 'N/A'}}</td>
              <td> {{ $data->email  ?? 'N/A'}}</td>
              <td>(+{{$data->country ?? 'N/A'}}) {{$data->phone_number}}</td>
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if(response.trim() == 'success') {
           ...
           $('#flash-message').html('Branch Restore Successfully');
...
$('body').on('click','.delete-button',function(e){
  ...
  swal({
    ...
    if (willDelete) {
      $.ajax({
        url: "{{ route('permanent_delete_branch') }}",
        type: 'post',
        data: {
          id: id
        }, 
