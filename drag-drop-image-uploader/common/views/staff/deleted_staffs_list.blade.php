...
@section('title', ' Super Admin | Deleted Staffs')
          ...
          <div class="card-body table p-0 mb-0">
            ...
            <table style="width:100%" id="users-list" class="table table-bordered table-hover yajra-datatable">
              <thead>
                ...
                @if(Gate::check('restore_staff') || Gate::check('permanent_deleted_staff'))
                  <th>Actions</th> 
               @endif
              ...
              <tbody>
                @foreach($staffsList as $key => $data)
               <tr> 
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if(response.trim() == 'success') {
         ...
         $('#flash-message').html('Staff Restore Successfully');
...
if (willDelete) {
  $.ajax({
    url: "{{ route('permanent_delete_staff') }}",
    type: 'post',
    data: {
      id: id
    }, 
    success: function(response) {
      ...
      if(response.trim() == 'success') {
         ...
         $('#flash-message').html('Staff Deleted Successfully');
         obj.parent().parent().remove();
         setTimeout(() => {
         $( "#flash-message" ).addClass("d-none");
         }, 5000);
