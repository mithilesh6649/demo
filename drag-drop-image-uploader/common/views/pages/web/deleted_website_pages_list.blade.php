        ...
        <div class="card-body">
          ...
          <table style="width:100%" id="users-list" class="table table-bordered table-hover yajra-datatable">
           ...
           <!--   @endif -->
            ...
            <tbody>
              @foreach($deletedWebsitePages as $key => $data)
             <tr> 
              <td> {{ $data->title_en  ?? ''}}</td>
              <td>{{ $data->title_ar  ?? ''}}</td>
...
swal({
title: "Are you sure?",
text: "Are you sure you want to restore this Content ?",
type: "warning",
showCancelButton: true,
}, function(willDelete) {
if (willDelete) {
  $.ajax({
    url: "{{ route('restore_website_pages') }}",
    type: 'post',
    data: {
      id: id
    },
    success: function(response) {
      ...
      if(response.trim() == 'success') {
         ...
         $('#flash-message').html('Content Restore Successfully');
...
if (willDelete) {
  $.ajax({
    url: "{{ route('permanent_website_pages') }}",
    type: 'post',
    data: {
      id: id
    }, 
