...
@section('title', ' Super Admin | Deleted Designations')
      ...
      <div class="card">
        <div class="card-main">
          <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
            <h3>Deleted Petty Expense Category</h3>
          </div>    
          <div class="card-body table p-0 mb-0">
            ...
            <table style="width:100%" id="users-list" class="table table-bordered table-hover yajra-datatable">
              <thead>
                ...
                @if(Gate::check('restore_petty_expense_category') || Gate::check('permanent_deleted_petty_expense_category'))
                  <th>Actions</th> 
               @endif
              ...
              <tbody>
                @foreach($DailyPettyExpenseCategory as $key => $data)
               <tr> 
                ...
                <td> {{ $data->cat_name ?? 'N/A'}}</td>
                        ...
                        <td>
                            ...
                            @can('restore_petty_expense_category')
                            ...
                            @endcan
                             ...
                             @can('permanent_deleted_petty_expense_category')
...
if (willDelete) {
  $.ajax({
    url: "{{ route('restore.expensecategory') }}",
    type: 'post',
    data: {
      id: id
    },
...
if (willDelete) {
  $.ajax({
    url: "{{ route('permanent.delete.expensecategory') }}",
    type: 'post',
    data: {
      id: id
    }, 
