...
@section('title', ' Super Admin | Brands')
   ...
   <div class="card">
    <div class="card-main">
     ...
     <div class="card-body table p-0 mb-0">
      ...
      <tbody>
       @forelse($DeletedBrands as $key => $data)
       <tr>
...
<script>
 ...
 $('body').on('click','.restore-button',function(e){
  ...
  swal({
    title: "Are you sure?",
    text: "Are you sure you want to restore this  Brands ?",
    type: "warning",
    showCancelButton: true,
  }, function(willDelete) {
    if (willDelete) {
      $.ajax({
        url: "{{ route('restore_brands') }}",
        type: 'post',
        data: {
          id: id
        },
        success: function(response) {
          ...
          if(response.trim() == 'success') {
           ...
           $('#flash-message').html(' Brands Restore Successfully');
...
//Permanent Delete
 ...
 $('body').on('click','.delete-button',function(e){
  ...
  swal({
    ...
    if (willDelete) {
      $.ajax({
        url: "{{ route('permanent_delete_brands') }}",
        type: 'post',
        data: {
          id: id
        }, 
        success: function(response) {
          ...
          if(response.trim() == 'success') {
           ...
           $('#flash-message').html(' Brands Deleted Successfully');
           obj.parent().parent().remove();
           setTimeout(() => {
             $( "#flash-message" ).addClass("d-none");
           }, 5000);
