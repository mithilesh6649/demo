...
@section('title', ' Super Admin | Deleted Cars')
                            ...
                            <table style="width:100%" id="users-list"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th>Model</th>
                                        <th>No Plate</th>
                                        @if (Gate::check('restore_cars') || Gate::check('permanent_delete_cars'))
                                            <th>Actions</th>
                                        @endif
                                ...
                                <tbody>
                                    @foreach ($allCars as $key => $data)
                                        <tr>

                                            <td> {{ $data->model ?? 'N/A' }}</td>

                                            <td> {{ $data->no_plate ?? 'N/A' }}</td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '.restore-button', function(e) {
                ...
                swal({
                    title: "Are you sure?",
                    text: "Are you sure you want to restore this Car ?",
                    type: "warning",
                    showCancelButton: true,
                }, function(willDelete) {
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('restore.cars') }}",
                            type: 'post',
                            data: {
                                id: id
                            },
            ...
            $('body').on('click', '.delete-button', function(e) {
                ...
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            url: "{{ route('permanent.delete.cars') }}",
                            type: 'post',
                            data: {
                                id: id
                            },
