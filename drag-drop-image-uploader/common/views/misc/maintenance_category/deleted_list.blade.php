...
@section('title', ' Super Admin | Deleted Maintenance Category')
      ...
      <div class="card">
        <div class="card-main">
          <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
            <h3>Deleted Maintenance Category</h3>
          </div>    
          <div class="card-body table p-0 mb-0">
            ...
            <table style="width:100%" id="users-list" class="table table-bordered table-hover yajra-datatable">
              <thead>
                ...
                @if(Gate::check('restore_maintenance_category') || Gate::check('permanent_deleted_maintenance_category'))
                  <th>Actions</th> 
               @endif
              ...
              <tbody>
                @foreach($MaintenanceCategory as $key => $data)
               <tr> 
                
                <td> {{ $data->cat_name ?? 'N/A'}}</td>
                        ...
                        <td>
                            ...
                            @can('restore_maintenance_category')
                            ...
                            @endcan
                             ...
                             @can('permanent_deleted_maintenance_category')
...
if (willDelete) {
  $.ajax({
    url: "{{ route('restore.MaintenanceCategory') }}",
    type: 'post',
    data: {
      id: id
    },
...
if (willDelete) {
  $.ajax({
    url: "{{ route('permanent.delete.MaintenanceCategory') }}",
    type: 'post',
    data: {
      id: id
    }, 
