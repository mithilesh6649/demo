 ...
 @section('title', 'Super Admin |   Discount')
                 ...
                 <div class="card">
                     <div class="card-main">
                         <div class="card-header alert d-flex justify-content-between align-items-center mx-0 pt-0">
                             <h3> Discount</h3>
                             @can('add_discount_offer')
                             <a class="btn btn-success" href="{{ route('discount.add') }}">Add Discount</a>
                             @endcan 
                         </div>
                             ...
                             <table style="width:100%" id="pages-list" class="table table-bordered table-hover">
                                 <thead>
                                     <tr>
                                         ...
                                         <th>End Date/ Time</th>
                                         <th>Status</th>
                        @if(Gate::check('view_discount_offer') || Gate::check('edit_discount_offer') || Gate::check('delete_discount_offer')) 
                        <th>Action</th>
                        @endif
                                     </tr>
                                 </thead>
                        ...
                        @if(Gate::check('view_discount_offer') || Gate::check('edit_discount_offer') || Gate::check('delete_discount_offer')) 
                                             ...
                                             @can('view_discount_offer')
                                                 <a class="action-button" title="View"
                                                     href="{{ route('discount.view', $discounts->id) }}"><i
                                                         class="text-info fa fa-eye"></i></a>
                                              
                                              @endcan
                                              @can('edit_discount_offer') 
                                                 <a href="{{ route('discount.edit', $discounts->id) }}" title="Edit"><i
                                                         class="text-warning fa fa-edit"></i></a>
                                                 @endcan
                                                 @can('delete_discount_offer')
                                                 <a class="action-button delete-button" title="Delete"
                                                     href="javascript:void(0)" data-id="{{ $discounts->id }}"><i
