...
class DiscountComplimentReturndownload
{
    ...
    public function result()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('A1', "ALL BRANCH MONTHLY DISCOUNT SALE-" . $this->filter_year)
        ...
        foreach ($allmonth as $key => $month) {
            ...
            foreach ($branch as $bkey => $branch_name) {
                if (isset($monthdiscount_check[$key])) {
                    if (in_array($bkey, array_keys($monthdiscount_check[$key]))) {

                        $total_sum = $total_sum + $monthdiscount_check[$key][$bkey]['discount'];

                        $discount = $monthdiscount_check[$key][$bkey]['discount'] == 0 ? '-' : number_format((float) $monthdiscount_check[$key][$bkey]['discount'], 3, '.', '');

                        $spreadsheet->getActiveSheet()->setCellValue($start_branch_alphabet . $counter, $discount);
                    } else {
        ...
        $fileName = 'Monthly-Discount-Compliment-Return-Sale' . $timestamp . '.xlsx';
