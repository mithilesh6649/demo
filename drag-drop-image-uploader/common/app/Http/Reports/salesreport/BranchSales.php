...
class BranchSales
{
    ...
    public function result()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ->mergeCells('A1:J1')
            ->setCellValue('A1', "MUGHAL MAHAL-BRANCH WISE GROSS SALES REPORT")
            ->getStyle("A1:J1")
            ->getFont()
            ->setSize(12);
        //->setItalic(true);
            ...
            if (in_array($aDates[$i - 1], array_keys($salesbranch))) {
                ...
                foreach ($branch_na as $key => $val) {
                    if (in_array($key, array_keys($salesbranch[$aDates[$i - 1]]))) {
                        ...
                        $DailySales->setCellValue($rowname[$j + 2] . ($i + 2), $salesbranch[$aDates[$i - 1]][$key]['gross_sale']);
                    } else {
        ...
        foreach (range('C', $rowname[count($rowArray) - 1]) as $rowval) {
            ...
            $COL62[] = '=' . $rowval . (count($aDates) + 9) . '*' . count($aDates);
        ...
        $fileName = 'Sales-By-Branch' . $timestamp . '.xlsx';
